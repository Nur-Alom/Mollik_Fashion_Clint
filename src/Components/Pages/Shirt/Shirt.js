...
const Shirt = () => {
    ...
    // Load Shirts Products.
    useEffect(() => {
        fetch('https://fakestoreapi.com/products')
            .then(res => res.json())
            .then(data => {
                const filterShirt = data.filter(dt => dt.category.includes("women's clothing"));
    ...
    return (
        <div className=''>
            <div className='text-center px-20 bg-slate-50 mb-20'>
                <div className='w-full bg-cover bg-no-repeat py-20 mb-32' style={{ backgroundImage: `url(${require('../../Images/shart-banner.jpg')})` }}>
                    <h1 className='font-sans font-bold text-5xl text-white'>T-SHIRTS</h1>
                </div>
                <div className='flex gap-6'>
                    <div className='bg-white w-1/5 h-fit p-4 rounded-md shadow-lg text-start'>
                        <div>
                            ...
                            <div className='px-2 mt-2 flex items-center border border-gray-400 rounded'>
                                ...
                                <input className='w-full pl-2 py-1 rounded-full outline-0 font-sans font-semibold text-base text-gray-800' type="search" name="shirtSearch" id="" placeholder='Search Products' />
                            </div>
                        </div>
                        <hr className='my-4' />
                    ...
                    <div className='w-4/5 grid grid-cols-4 gap-5'>
                        {
                            shirts.map(shirt => <div key={shirt.id} className='shadow-2xl bg-white rounded-lg'>
                                <NavLink to={`/product/${shirt.id}`}>
                                    <div className=''>
                                        <img className='w-52 h-60 mx-auto p-5' src={shirt.image} alt="" />
                                    </div>
                                    <div className='pt-4 px-5 text-left'>
                                        <h1 title={shirt.title} className='font-sans font-semibold text-base'>{shirt.title.slice(0, 15)}...</h1>
                                        <p className='pb-2 font-sans font-semibold text-sm text-gray-500'>{shirt.description.slice(0, 15)}</p>
                                        <span className='font-sans font-semibold text-sm text-center text-red-600'>Price: {shirt.price}</span>
                                    </div>
                                </NavLink>
